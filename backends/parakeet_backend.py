...
from app_config import AUDIO_RUNTIME, PARAKEET_DEFAULT_MODEL_ID, PARAKEET_MODEL_MAP
...
class ParakeetBackend(WhisperBackend):
    ...
    def get_model_map(self) -> dict:
        ...
        return PARAKEET_MODEL_MAP.copy()
    ...
    def load_model(
        ...
        """
        logging.getLogger(__name__).debug(f"[Parakeet] load_model called with:")
        logging.getLogger(__name__).debug(f"  model_key: {model_key}")
        logging.getLogger(__name__).debug(f"  device: {device}")
        logging.getLogger(__name__).debug(f"  compute_type: {compute_type}")
        logging.getLogger(__name__).debug(f"  model_path: {model_path}")
        
        logging.getLogger(__name__).debug("[Parakeet] Importing torch...")
        import torch
        logging.getLogger(__name__).debug(f"[Parakeet] torch imported. CUDA available: {torch.cuda.is_available()}")
        
        logging.getLogger(__name__).debug("[Parakeet] Importing nemo.collections.asr...")
        import nemo.collections.asr as nemo_asr
        logging.getLogger(__name__).info("[Parakeet] NeMo ASR imported successfully")
        
        # Map friendly names to model IDs
        model_map = self.get_model_map()
        logging.getLogger(__name__).debug(f"[Parakeet] Model map: {model_map}")
        
        # First check if model_key is directly in our map
        if model_key in model_map:
            model_id = model_map[model_key]
            logging.getLogger(__name__).debug(f"[Parakeet] Found model_key in map -> model_id: {model_id}")
        # Also check if it's a HuggingFace ID we recognize
        elif model_key in model_map.values():
            model_id = model_key
            logging.getLogger(__name__).debug(f"[Parakeet] model_key is a HF ID -> model_id: {model_id}")
        # Check if it's the Parakeet model from GUI's MODEL_MAP
        elif "nvidia/parakeet" in model_key:
            model_id = model_key
            logging.getLogger(__name__).debug(f"[Parakeet] model_key contains nvidia/parakeet -> model_id: {model_id}")
        else:
            # Default to the latest v3 model
            model_id = PARAKEET_DEFAULT_MODEL_ID
            logging.getLogger(__name__).debug(f"[Parakeet] Using default model_id: {model_id}")
        
        # Set device
        if device == "cuda" and torch.cuda.is_available():
            map_location = "cuda"
            logging.getLogger(__name__).debug(f"[Parakeet] Using CUDA device")
        else:
            map_location = "cpu"
            device = "cpu"  # Fallback to CPU if CUDA requested but not available
            logging.getLogger(__name__).debug(f"[Parakeet] Using CPU device")
        ...
        if model_path:
            logging.getLogger(__name__).debug(f"[Parakeet] Checking model_path: {model_path}")
            # Only use local path if it's explicitly a .nemo file
            if model_path.endswith('.nemo') and os.path.isfile(model_path):
                use_local_nemo = True
                logging.getLogger(__name__).debug(f"[Parakeet] Using local .nemo file: {model_path}")
            # Or if it contains a .nemo file directly
            elif os.path.isdir(model_path):
                try:
                    nemo_files = [f for f in os.listdir(model_path) if f.endswith('.nemo')]
                    logging.getLogger(__name__).debug(f"[Parakeet] Found .nemo files in dir: {nemo_files}")
                    if nemo_files:
                        model_path = os.path.join(model_path, nemo_files[0])
                        use_local_nemo = True
                        logging.getLogger(__name__).debug(f"[Parakeet] Using local .nemo file: {model_path}")
                except Exception as e:
                    logging.getLogger(__name__).warning(f"[Parakeet] Error scanning model_path: {e}")
            else:
                logging.getLogger(__name__).debug(f"[Parakeet] model_path exists but is not .nemo file or dir with .nemo")
        else:
            logging.getLogger(__name__).debug(f"[Parakeet] No model_path provided, will download from HuggingFace")
        
        logging.getLogger(__name__).debug(f"[Parakeet] use_local_nemo: {use_local_nemo}")
        
        if use_local_nemo:
            # Load from local .nemo file
            logging.getLogger(__name__).info(f"[Parakeet] Loading from local .nemo file: {model_path}")
            ...
            logging.getLogger(__name__).info(f"[Parakeet] Loading from HuggingFace: {model_id}")
            logging.getLogger(__name__).debug(f"[Parakeet] Calling ASRModel.from_pretrained(model_name='{model_id}', map_location='{map_location}')")
            self._model = nemo_asr.models.ASRModel.from_pretrained(
                model_name=model_id,
                map_location=map_location
            )
            logging.getLogger(__name__).info(f"[Parakeet] Model loaded from HuggingFace successfully")
        
        # Move to appropriate device
        logging.getLogger(__name__).debug(f"[Parakeet] Moving model to device: {device}")
        if device == "cuda":
        ...
        logging.getLogger(__name__).debug(f"[Parakeet] Setting model to eval mode")
        self._model.eval()
        
        # Enable half-precision if requested and on CUDA
        if device == "cuda" and compute_type == "float16":
            logging.getLogger(__name__).debug(f"[Parakeet] Converting to half precision (float16)")
            self._model = self._model.half()
        
        self._current_model_key = model_key
        self._current_device = device
        
        logging.getLogger(__name__).info(f"[Parakeet] Model loading complete!")
        if progress_callback:
    ...
    def transcribe(
        ...
        """
        logging.getLogger(__name__).debug(f"[Parakeet] transcribe() called with audio_path: {audio_path}")
        ...
        logging.getLogger(__name__).debug(f"[Parakeet] Audio file exists, size: {os.path.getsize(audio_path)} bytes")
        
        # NeMo's TDT/RNNT models use a simpler transcribe API
        # They don't support beam_size - that's for CTC models
        logging.getLogger(__name__).debug(f"[Parakeet] Calling model.transcribe() with timestamps=True")
        try:
            output = self._model.transcribe([audio_path], timestamps=True)
            logging.getLogger(__name__).debug(f"[Parakeet] Transcription returned: {type(output)}")
        except TypeError as e:
            # If timestamps parameter is not supported, try without it
            logging.getLogger(__name__).warning(f"[Parakeet] timestamps parameter failed: {e}, trying without")
        ...
        if output and len(output) > 0:
            ...
            logging.getLogger(__name__).debug(f"[Parakeet] Transcription type: {type(transcription)}")
            logging.getLogger(__name__).debug(f"[Parakeet] Transcription value: {transcription}")
            
            # Check if we have segment-level timestamps
            if hasattr(transcription, 'timestamp') and transcription.timestamp:
                logging.getLogger(__name__).debug(f"[Parakeet] Has timestamps: {transcription.timestamp}")
                ...
                logging.getLogger(__name__).debug(f"[Parakeet] Using text attribute: {text}")
                result.append(TranscriptionSegment(
                    text=text,
                    start=0.0,
                    end=0.0
                ...
                logging.getLogger(__name__).debug(f"[Parakeet] Using str() fallback: {text}")
                result.append(TranscriptionSegment(
                    text=text,
                    start=0.0,
                    end=0.0
        ...
        logging.getLogger(__name__).info(f"[Parakeet] Returning {len(result)} segments")
        return result
